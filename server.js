...
app.post("/api/upload", upload.single("file"), async (req, res) => {
  try {
    ...
    const isMp3 = req.body.isMP3 === "true";

    if (!isMp3) {
      // File needs to be converted to MP3
      ...
      // File is already in MP3 format
      const gcsUri = `gs://podship/${req.file.filename}`;
    ...
    try {
      console.log("bad  configuration, converting to mp3");
      const client = new speech.SpeechClient();
      const inputFile = req.file.path;
      // File needs to be converted to MP3
      const randomString = generateRandomString();
